...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if record.levelname == 'CRITICAL':
            return colored(log_message, self.COLORS.get(record.levelname), attrs=['bold'])
...
def def_log(path_log:str= './', file:str='log.log'):
    ...
    formatter_console = ColoredFormatter('%(asctime)s  %(levelname)-8s(%(name)s):     %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
