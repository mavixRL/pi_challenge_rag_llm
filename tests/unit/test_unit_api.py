...
def test_get_context():
    ...
    try:
        ...
        assert 'documents' in context[0], "La clave 'documents' no está en el contexto[0]"
        assert len(context[0]['documents']) > 0, "La lista de documentos está vacía"
        assert context[0]['documents'][0][0] == config.get(['test_get_context', 'context']), "El contenido del documento no coincide"
...
def test_get_response():
    ...
    try:
        assert response is not None, "La respuesta es None"
        assert response == config.get(['test_get_response','response']), "La respuesta no coincide"
